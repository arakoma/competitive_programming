...
TOKEN = os.getenv('TOKEN', 'e2eea0dac9b9b827d4cf0e8e9569ffc1')
...
def main():
    while True:
        ...
        game_id, remaining_ms = map(int, game_resp.split())
        ...
        for row in range(20):
            for col in range(20):
                stage_points.append((row, col, stage[row][col]))
        
        stage_points.sort(key=lambda x: x[2], reverse=True)
        #stage_points = set(stage_points)
        idx = 0
        while True:
            """
